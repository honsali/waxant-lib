...
const DialogueOuiNon = ({ visible, nom, libelle = null, icone = null, entete = null, nomActionOui = 'accepter', nomActionNon = 'refuser', actionOui, actionNon, nomActionAnnuler = 'annuler', actionAnnuler = null, largeur = 520, ridOui = null, ridNon = null, children }) => {
    ...
    const getFooter = () => {
        return (
            <Space>
                <BoutonContourPrimaire nom={nomActionAnnuler} action={actionAnnuler} inactif={(ridOui || ridNon) && ''} />
                <BoutonPleinPrimaire nom={nomActionNon} contexte={nom} action={actionNon} rid={ridNon} inactif={(ridOui || ridNon) && ''} />
                <BoutonPleinPrimaire nom={nomActionOui} contexte={nom} action={actionOui} rid={ridOui} inactif={(ridOui || ridNon) && ''} />
            </Space>
        );
    };
